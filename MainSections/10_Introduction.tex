\chapter{GİRİŞ} \label{chap:introduction}
{
...
Bu çalışma, yalnızca teorik bir sensör füzyonu tekniği sunmakla kalmamakta; aynı zamanda Citrus Farm gibi veri setleri üzerinde test edilerek performans kazanımları detaylı bir şekilde ortaya konmaktadır. Buna ek olarak, saha testleri için oluşturulan ZED Stereo Kamera, Velodyne VLP16 LiDAR ve NVIDIA Jetson Nano bileşenlerinden oluşan taşınabilir bir sensör platformu ile gerçek zamanlı veri toplanmış; böylece önerilen yöntemin gerçek zamanlı çalışma koşullarındaki etkisi de analiz edilmiştir.
}
...
\begin{itemize}
    ...
    \item \textbf{Hazır veri seti üzerinde SLAM odometri karşılaştırmaları:} Citrus Farm Veri Seti~\cite{citrus_farm_dataset} ile yapılan çalışmalar ile kendi toplayacağımız verinin hangi formatta olması gerektiği daha iyi tespit edilmiştir. Aynı zamanda bu veri setinin de gerçek veriler içermesi sayesinde algoritmanın gerçek veri üzerinde de iyi sonuçlar verdiği matematiksel kanıtlarla sunulmuştur.
...
\begin{itemize}
    \item \autoref{chap:introduction}'de konuya giriş yapılmıştır. Ayrıca tez konusundaki amacımız ve motivasyonumuz yine burada belirtilmiştir.
    \item \autoref{chap:background}'de tezde kullanılan matematik ve algoritmalar ile ilgili genel bir arka plan bilgisi verilmiştir.
    \item \autoref{chap:related-work}'te literatürde bu konu ile ilgili daha önce yapılan çalışmalar anlatılmış, bu çalışmaların varsa tez ile ilgili kısımları detaylandırılmıştır.
    \item \autoref{chap:hardware}'te tez sürecinde donanım üzerinde yapılan çalışmalar ve bununla elde edilen veri seti anlatılmıştır.
    \item \autoref{chap:metodology}'te metodolojji tüm detaylarıyla anlatılmış ve algoritmanın matematiksel denklemleri verilmiştir.
    \item \autoref{chap:experimentation}'de SensorSuiteV2 ve CitrusFarm Veriseti üzerinde PG ön işlemeli ve ön işlemesiz SLAM sonuçları karşılaştırılmış ve elde edilen sonuçlar gösterilmiştir.
    \item \autoref{chap:coupled}'de denenen bağlaşık metotlar açıklanmış ve sonuçları paylaşılmıştır. Ayrıca ileride bu konuda yapılabilinecek çalışmalar bu bölümde tartışılmıştır.
    \item \autoref{chap:conclusion}'da çalışma soncunda elde edilen sonuçlar değerlendirilmiştir.
\end{itemize}
