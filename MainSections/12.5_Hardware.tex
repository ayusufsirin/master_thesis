...
\subsubsection{Kapalı Alan Veri Seti}
Kapalı alan veri setini, Hacettepe Üniversitesi Elektrik ve Elektronik Mühendisliği Bölümü içerisinde SSv1'i kullanarak topladık. Deneyi bir koridorda yatay yönde 45 cm aralıklarla 3 sıra halinde,  dikey yönde 90 cm aralıklarla 10 m boyunca sıraladığımız ArUco işaretleyicileri arasında uzaktan kontrol ettiğimiz bacaklı robotu kullanarak aldık. Deney ortamının Web Kamerası görüntüsü Şekil (\ref{aruco_webcam})'de görülebilir.

\begin{figure}[!htb]
    \centering
    \begin{adjustbox}{width=\textwidth * 10 /12}
        \includegraphics{Figures/aruco_ss.png}
    \end{adjustbox}
    \caption{Kapalı Alan Deney Ortamı}
    \label{aruco_webcam}
\end{figure}

Deneyleri iki farklı günde aldık ve bunları gün ve alınma sırasına göre isimlendirdik. Ayın 7'sinde alınan deneyler uzaktan kumanda ile alındığı için hız komutlarında gürültüye sahiplerdir. Ayın 11'inde aldığımız deneyleri ise robota sabit bir hız komutu göndererek aldık, bu nedenle yapılan hareket daha düzgündür.

Toplanılar veriler;
\begin{itemize}
    \item Stereo Kamera (ZED)
        \begin{itemize}
            \item Sağ ve Sol RGBD Görüntü
            \item Sağ ve Sol Gri Ölçekli Görüntü
            \item Derinlik Görüntüsü
            \item Görsel Odometri
        \end{itemize}
    \item LiDAR (VLP-16)
        \begin{itemize}
            \item Nokta Bulutu
            \item LiDAR Odometrisi
        \end{itemize}
    \item Web Kamerası ve ArUco İşaretleyiciler
        \begin{itemize}
            \item Referans Pozisyon
            \item Deney Videosu 
        \end{itemize}
\end{itemize}
olarak sınıflandırılabilinir. 

Verileri, ROS Melodic ortamında, Stereolabs'ın ZED kamera için yayınladığı ROS paketi\cite{zed_ros_wrapper}, Velodyne'ın VLP-16 LiDAR için yayınladığı ROS paketi\cite{velodyne_ros_drivers}, LeGO-LOAM\cite{legoloam} makalesi ile birlikte yayınlanan ROS paketi\cite{lego_loam_ros} ve kendi yazdığımız ArUco ROS paketini\cite{aruco_hu_neurolab} kullanılarak elde ettik. Bu verileri yine ROS ortamında .bag formatında kaydettik.
